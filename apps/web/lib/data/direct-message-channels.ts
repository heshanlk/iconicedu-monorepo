import type { ChannelVM } from '@iconicedu/shared-types';
import {
  MOCK_EDUCATOR_1,
  MOCK_EDUCATOR_2,
  MOCK_EDUCATOR_3,
  MOCK_EDUCATOR_4,
  MOCK_EDUCATOR_5,
  MOCK_GUARDIAN,
  MOCK_ORG_ID,
} from './people';

export const DIRECT_MESSAGE_CHANNELS: ChannelVM[] = [
  {
    id: '7a111111-1111-4111-8111-111111111111',
    orgId: MOCK_ORG_ID,
    kind: 'dm',
    topic: MOCK_EDUCATOR_1.profile.displayName,
    topicIconKey: null,
    description: 'Direct message thread',
    visibility: 'private',
    purpose: 'general',
    status: 'active',
    createdBy: MOCK_GUARDIAN.ids.id,
    createdAt: '2025-01-05T12:00:00.000Z',
    archivedAt: null,
    postingPolicy: {
      kind: 'members-only',
      allowThreads: true,
      allowReactions: true,
    },
    headerItems: [
      { key: 'last-seen', label: '5m' },
      { key: 'saved', label: 'Saved' },
      { key: 'info', label: 'Info', isPrimary: true },
    ],
    dmKey: `dm:${MOCK_EDUCATOR_1.ids.id}-${MOCK_GUARDIAN.ids.id}`,
    context: {
      primaryEntity: { kind: 'user', id: MOCK_EDUCATOR_1.ids.id },
    },
    participants: [MOCK_GUARDIAN, MOCK_EDUCATOR_1],
    messages: { items: [], total: 0 },
    media: { items: [], total: 0 },
    files: { items: [], total: 0 },
    readState: {
      channelId: '7a111111-1111-4111-8111-111111111111',
      lastReadAt: '2025-01-10T19:00:00.000Z',
      unreadCount: 2,
    },
  },
  {
    id: '7a222222-2222-4222-8222-222222222222',
    orgId: MOCK_ORG_ID,
    kind: 'dm',
    topic: MOCK_EDUCATOR_2.profile.displayName,
    topicIconKey: null,
    description: 'Direct message thread',
    visibility: 'private',
    purpose: 'general',
    status: 'active',
    createdBy: MOCK_GUARDIAN.ids.id,
    createdAt: '2025-01-05T12:05:00.000Z',
    archivedAt: null,
    postingPolicy: {
      kind: 'members-only',
      allowThreads: true,
      allowReactions: true,
    },
    headerItems: [
      { key: 'last-seen', label: '12m' },
      { key: 'saved', label: 'Saved' },
      { key: 'info', label: 'Info', isPrimary: true },
    ],
    dmKey: `dm:${MOCK_EDUCATOR_2.ids.id}-${MOCK_GUARDIAN.ids.id}`,
    context: {
      primaryEntity: { kind: 'user', id: MOCK_EDUCATOR_2.ids.id },
    },
    participants: [MOCK_GUARDIAN, MOCK_EDUCATOR_2],
    messages: { items: [], total: 0 },
    media: { items: [], total: 0 },
    files: { items: [], total: 0 },
    readState: {
      channelId: '7a222222-2222-4222-8222-222222222222',
      lastReadAt: '2025-01-10T18:30:00.000Z',
      unreadCount: 1,
    },
  },
  {
    id: '7a333333-3333-4333-8333-333333333333',
    orgId: MOCK_ORG_ID,
    kind: 'dm',
    topic: MOCK_EDUCATOR_3.profile.displayName,
    topicIconKey: null,
    description: 'Direct message thread',
    visibility: 'private',
    purpose: 'general',
    status: 'active',
    createdBy: MOCK_GUARDIAN.ids.id,
    createdAt: '2025-01-05T12:10:00.000Z',
    archivedAt: null,
    postingPolicy: {
      kind: 'members-only',
      allowThreads: true,
      allowReactions: true,
    },
    headerItems: [
      { key: 'last-seen', label: '1h' },
      { key: 'saved', label: 'Saved' },
      { key: 'info', label: 'Info', isPrimary: true },
    ],
    dmKey: `dm:${MOCK_EDUCATOR_3.ids.id}-${MOCK_GUARDIAN.ids.id}`,
    context: {
      primaryEntity: { kind: 'user', id: MOCK_EDUCATOR_3.ids.id },
    },
    participants: [MOCK_GUARDIAN, MOCK_EDUCATOR_3],
    messages: { items: [], total: 0 },
    media: { items: [], total: 0 },
    files: { items: [], total: 0 },
    readState: {
      channelId: '7a333333-3333-4333-8333-333333333333',
      lastReadAt: '2025-01-10T17:50:00.000Z',
      unreadCount: 3,
    },
  },
  {
    id: '7a444444-4444-4444-8444-444444444444',
    orgId: MOCK_ORG_ID,
    kind: 'dm',
    topic: MOCK_EDUCATOR_4.profile.displayName,
    topicIconKey: null,
    description: 'Direct message thread',
    visibility: 'private',
    purpose: 'general',
    status: 'active',
    createdBy: MOCK_GUARDIAN.ids.id,
    createdAt: '2025-01-05T12:15:00.000Z',
    archivedAt: null,
    postingPolicy: {
      kind: 'members-only',
      allowThreads: true,
      allowReactions: true,
    },
    headerItems: [
      { key: 'last-seen', label: '2h' },
      { key: 'saved', label: 'Saved' },
      { key: 'info', label: 'Info', isPrimary: true },
    ],
    dmKey: `dm:${MOCK_EDUCATOR_4.ids.id}-${MOCK_GUARDIAN.ids.id}`,
    context: {
      primaryEntity: { kind: 'user', id: MOCK_EDUCATOR_4.ids.id },
    },
    participants: [MOCK_GUARDIAN, MOCK_EDUCATOR_4],
    messages: { items: [], total: 0 },
    media: { items: [], total: 0 },
    files: { items: [], total: 0 },
    readState: {
      channelId: '7a444444-4444-4444-8444-444444444444',
      lastReadAt: '2025-01-10T16:45:00.000Z',
      unreadCount: 0,
    },
  },
  {
    id: '7a555555-5555-4555-8555-555555555555',
    orgId: MOCK_ORG_ID,
    kind: 'dm',
    topic: MOCK_EDUCATOR_5.profile.displayName,
    topicIconKey: null,
    description: 'Direct message thread',
    visibility: 'private',
    purpose: 'general',
    status: 'active',
    createdBy: MOCK_GUARDIAN.ids.id,
    createdAt: '2025-01-05T12:20:00.000Z',
    archivedAt: null,
    postingPolicy: {
      kind: 'members-only',
      allowThreads: true,
      allowReactions: true,
    },
    headerItems: [
      { key: 'last-seen', label: '1d' },
      { key: 'saved', label: 'Saved' },
      { key: 'info', label: 'Info', isPrimary: true },
    ],
    dmKey: `dm:${MOCK_EDUCATOR_5.ids.id}-${MOCK_GUARDIAN.ids.id}`,
    context: {
      primaryEntity: { kind: 'user', id: MOCK_EDUCATOR_5.ids.id },
    },
    participants: [MOCK_GUARDIAN, MOCK_EDUCATOR_5],
    messages: { items: [], total: 0 },
    media: { items: [], total: 0 },
    files: { items: [], total: 0 },
    readState: {
      channelId: '7a555555-5555-4555-8555-555555555555',
      lastReadAt: '2025-01-09T18:05:00.000Z',
      unreadCount: 0,
    },
  },
];
