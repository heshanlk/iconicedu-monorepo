...
- Follow the existing `apps/web/lib` layout—do not introduce new folder patterns. Each entity (user, org, class-space, learning-space, etc.) deserves its own bundle with explicit `queries`, `mappers`, and `builders` subfolders under `apps/web/lib/<entity>`. Keep fixtures in `lib/data`, sidebar wiring in `lib/sidebar`, and keep the entity bundles focused on reusable data access or transformation logic.
- Create or extend row definitions, view models, and helpers in `packages/shared-types` before using them in apps. Once the shared contract exists, mirror it under the matching `apps/web/lib/<entity>` bundle so every entity follows the same `queries` → `mappers` → `builders` pipeline. This keeps expectations clear for downstream agents and prevents duplicate work.
